...
@if (count($providers))
    <div class="grid gap-3 sm:grid-cols-2" aria-label="Masuk dengan akun sosial">
                ...
                <span class="grid h-5 w-5 place-items-center" aria-hidden="true">
                    @switch($provider)
                        ...
                        @case(\App\Enums\SocialProvider::Apple)
                            <svg viewBox="0 0 24 24" class="h-5 w-5 fill-current" aria-hidden="true"><path d="M16.71 12.68c.02-1.7 1.39-2.52 1.45-2.56-.79-1.16-2.01-1.32-2.44-1.34-1.04-.1-2.04.61-2.57.61-.53 0-1.35-.6-2.22-.58-1.14.02-2.2.66-2.79 1.68-1.2 2.08-.31 5.16.86 6.85.57.83 1.25 1.76 2.14 1.73.86-.03 1.18-.55 2.22-.55s1.33.55 2.24.53c.92-.02 1.51-.84 2.08-1.67.66-.96.93-1.88.95-1.93-.02-.01-1.81-.7-1.79-2.77Zm-1.7-4.98c.48-.58.8-1.39.71-2.2-.69.03-1.53.46-2.03 1.04-.45.52-.84 1.35-.73 2.14.77.06 1.56-.39 2.05-.98Z"/></svg>
                            @break
                        @case(\App\Enums\SocialProvider::Microsoft)
                            <svg viewBox="0 0 24 24" class="h-5 w-5" aria-hidden="true"><path fill="#f25022" d="M2 2h9.5v9.5H2z"/><path fill="#7fba00" d="M12.5 2H22v9.5h-9.5z"/><path fill="#00a4ef" d="M2 12.5h9.5V22H2z"/><path fill="#ffb900" d="M12.5 12.5H22V22h-9.5z"/></svg>
                            @break
                        @case(\App\Enums\SocialProvider::Facebook)
                            <svg viewBox="0 0 24 24" class="h-5 w-5 fill-[#1877f2]" aria-hidden="true"><path d="M24 12.07C24 5.4 18.63 0 12 0S0 5.4 0 12.07C0 18.1 4.39 23.1 10.13 24v-8.44H7.08v-3.49h3.05V9.41c0-3.03 1.79-4.7 4.54-4.7 1.31 0 2.69.24 2.69.24v2.97h-1.52c-1.5 0-1.97.94-1.97 1.9v2.25h3.35l-.54 3.49h-2.81V24C19.61 23.1 24 18.1 24 12.07Z"/></svg>
                            @break
                        @case(\App\Enums\SocialProvider::X)
                            <svg viewBox="0 0 24 24" class="h-5 w-5 fill-current" aria-hidden="true"><path d="M18.9 2H22l-6.77 7.74L23.2 22h-6.24l-4.89-7.57L5.45 22H2.34l7.24-8.27L1.93 2h6.4l4.42 6.89L18.9 2Zm-1.09 18h1.72L7.4 3.9H5.55L17.81 20Z"/></svg>
                    @endswitch
                </span>
                <span>Lanjutkan dengan {{ $provider->label() }}</span>
            </a>
        @endforeach
    </div>
